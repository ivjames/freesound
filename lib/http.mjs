 ...
 *
 * nginx is in front, so the socket address is always the loopback proxy and the
 * real client has to come from a header. Which header, and which part of it,
 * decides whether the rate limit works at all:
 *
 * `provision-site` writes BOTH `X-Real-IP $remote_addr` and `X-Forwarded-For
 * $proxy_add_x_forwarded_for`. Only the first is trustworthy. `$proxy_add_x_-
 * forwarded_for` expands to "$http_x_forwarded_for, $remote_addr" — it APPENDS
 * to whatever the client sent, so a request carrying `X-Forwarded-For: 1.2.3.4`
 * arrives here as "1.2.3.4, <real ip>". Reading the first entry therefore reads
 * a value the attacker chose: a fresh one per request defeats the limiter
 * completely and grows its map without bound. The last entry is the one nginx
 * appended itself.
 *
 * So: X-Real-IP first, the LAST X-Forwarded-For entry second, socket last. With
 * no proxy headers at all the limit degrades to global rather than per-client —
 * degraded, not bypassed, which is the right direction to fail.
 */
