...
export class Translator {
   ...
   */
  async translate(items, { budgetMs = 0 } = {}) {
    ...
    for (let i = 0; i < todo.length; i += MAX_BATCH) {
      ...
      if (remaining <= 0) {
        console.warn('[translate] out of budget; remaining titles keep their original names');
        break;
      }
      const batch = todo.slice(i, i + MAX_BATCH);
      let results;
      try {
        results = await this.#callModel(client, batch, remaining);
      } catch (err) {
  ...
  async #callModel(client, batch, budgetMs = 0) {
    // A per-request override rather than a client-level one: the client is
    // shared, and the budget belongs to the caller's deadline. maxRetries drops
    // to 0 under a budget because a retry would blow it by definition.
    //
    // Must be a finite integer: with no budget the caller's deadline is
    // Infinity, and handing that to the SDK is a hard "timeout must be an
    // integer" that would have turned every unbudgeted translation into a
    // silent no-op.
    const bounded = Number.isFinite(budgetMs) && budgetMs > 0;
    const options = bounded ? { timeout: Math.max(1, Math.floor(budgetMs)), maxRetries: 0 } : undefined;
