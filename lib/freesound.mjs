...
export const SOUND_FIELDS =
  'id,name,url,username,license,duration,previews,images,tags,pack,avg_rating,num_downloads';
...
/**
 * Sounds Freesound considers acoustically similar to `id`, from its own
 * similarity index over content descriptors — which is the only way to answer
 * "do I already have something that sounds like this?", since nothing about
 * the title or the uploader tells you.
 *
 * One request per clip checked, so callers rate-limit it rather than calling it
 * per search result. `fields` is kept minimal here: the ids are what matter,
 * and the names are only for showing what matched.
 */
export function similar(apiKey, id, { pageSize = 15 } = {}) {
  if (!Number.isInteger(id) || id <= 0) throw new FreesoundError('Bad sound id', 400);
  return call(apiKey, `/sounds/${id}/similar/`, {
    fields: 'id,name,username,url,license,duration',
    page_size: Math.max(1, Math.min(MAX_PAGE_SIZE, pageSize)),
  });
}

/**
 * The pack a sound belongs to, as a stable key.
 *
 * Freesound returns `pack` as a URI to the pack's API resource
 * (".../apiv2/packs/1234/"), so the id is pulled out of it — the full URI works
 * as a grouping key too, but the id is what a UI can show and a log can read.
 */
export function packId(pack) {
  if (!pack) return '';
  const m = /\/packs\/(\d+)\/?$/.exec(String(pack));
  return m ? m[1] : String(pack).slice(0, 80);
}
...
export function toSound(raw) {
  ...
  return {
    ...
    // Used to group a pack's near-identical takes in search results. Empty for
    // a standalone upload, which is most of them.
    pack: packId(raw?.pack),
  };
}
